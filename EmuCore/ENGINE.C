...
UINT SwitchToState(UINT nNewState)
{
	...
	case SM_RUN: // Run
		...
		case SM_SLEEP: // -> Sleep
			...
			break;
        case SM_INACTIVE: // -> Inactive
            nNextState = SM_INACTIVE;
            bInterrupt = TRUE;              // exit main opcode loop
            SuspendDebugger();              // suspend debugger
            SetEvent(hEventShutdn);         // wake thread if it is in SHUTDN
    ...
    case SM_INACTIVE: // Inactive
        ...
        case SM_RUN: // -> Run
            nNextState = SM_RUN;
            bInterrupt = (nDbgState == DBG_OFF) &&
                         (Chipset.Shutdn || Chipset.SoftInt);
            ResumeDebugger();
            SetEvent(hEventShutdn);      // wake inactive worker
            break;
...
VOID SetApplicationActive(BOOL active)
{
    if (active)
    {
        if (nState == SM_INACTIVE)
            SwitchToState(SM_RUN);
    }
    else
    {
        if (nState == SM_RUN)
            SwitchToState(SM_INACTIVE);
    }
}
