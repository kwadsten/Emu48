...
- (id)initWithScale:(unsigned)aScale colors:(NSDictionary *)aColors
    {
        return [self initWithScaleX:aScale
                               scaleY:aScale
                              colors:aColors];
    }

// v1.68 changes
- (id)initWithScaleX:(CGFloat)aScaleX
               scaleY:(CGFloat)aScaleY
              colors:(NSDictionary *)aColors
{
    lcdScaleX = (aScaleX > 0) ? aScaleX : 1;
    lcdScaleY = (aScaleY > 0) ? aScaleY : 1;

    CGSize size = computeLCDSize(lcdScaleX, lcdScaleY);
    ...
    // fix initialization v1.68
    if (rep)
    {
        img = [[NSImage alloc] initWithSize:
                   NSMakeSize([rep pixelsWide], [rep pixelsHigh])];
        ...
        if (img)
        {
            ...
            // Get the bitmap pointer while rep is still valid.
            lcd = [rep bitmapData];
...
- (void)drawRect:(NSRect)rect
{
    ...
    NSRect srcRect = NSMakeRect(0, 0, 131, 64);

    NSBitmapImageRep *rep =
        (NSBitmapImageRep *)[[img representations] firstObject];
    ...
    // Draw red outline around lcd area for debugging
...
//    [NSBezierPath strokeRect:self.bounds];
    ...
    if (inactiveOverlay)
    {
        ...
        CGFloat fontSize = bounds.size.width * (20.0 / 262.0);
        ...
        NSString *message = @"Emulation Paused";
        NSSize textSize = [message sizeWithAttributes:attributes];

        NSPoint point = NSMakePoint(
            inactiveOverlayDisplayCenter.x - textSize.width / 2.0,
            inactiveOverlayDisplayCenter.y - textSize.height / 2.0
        );
        
        [message drawAtPoint:point
...
- (void)setInactiveOverlayDisplayRect:(NSRect)rect
{
    inactiveOverlayDisplayRect = rect;
    [self setNeedsDisplay:YES];
}

- (void)setInactiveOverlayDisplayCenter:(NSPoint)center
{
    inactiveOverlayDisplayCenter = center;
    [self setNeedsDisplay:YES];
}
...
{
    NSLog(@"RAW LCD MOUSE DOWN: point=%@ frame=%@",
          NSStringFromPoint(
...
- (void)mouseDragged:(NSEvent *)theEvent
{
    ...
    dragRect.size.width  *= lcdScaleX;
    dragRect.size.height *= lcdScaleY;
