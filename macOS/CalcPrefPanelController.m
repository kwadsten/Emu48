...
- (IBAction)prefChangeStartupCalculator:(id)sender
{
    ...
    [[gallery window] setTitle:@"Choose a startup calculator:"];
    [gallery setActionButtonTitle:@"Choose"];
    [gallery setDelegate:self];

    calculatorGallery = [gallery retain];

    NSWindow *galleryWindow = [gallery window];

    [[self window]
        beginSheet:galleryWindow
        completionHandler:^(NSModalResponse returnCode)
        {
            [calculatorGallery release];
            calculatorGallery = nil;
        }];
...
- (id)init
{
    ...
    if (self)
    {
        ...
        [[NSNotificationCenter defaultCenter]
            addObserver:self
               selector:@selector(startupCalculatorChanged:)
                   name:@"StartupCalculatorDidChange"
                 object:[CalcManager sharedManager]];

        [self window];
        [self startupCalculatorChanged:nil];
    }
    return self;
}
...
- (void)startupCalculatorChanged:(NSNotification *)notification
{
    NSDictionary *calc =
        [[CalcManager sharedManager] startupCalculator];

    NSString *title = @"Use calculator:";

    if (calc)
    {
        NSString *calcTitle = calc[@"title"];

        if ([calcTitle length] > 0)
        {
            title =
                [NSString stringWithFormat:
                    @"Use calculator: %@", calcTitle];
        }
    }

    [[startupModeMatrix cellAtRow:kStartupCalculator column:0]
        setTitle:title];

    [startupModeMatrix setNeedsDisplay:YES];
}
