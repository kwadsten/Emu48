...
- (void)InitGlobal:(KmlBlock *)pBlock
{
    ...
    while (pLine)
    {
        switch (pLine->eCommand)
        {
            ...
            // v1.68 changes
            case TOK_SCALE:
                if ([delegate respondsToSelector:@selector(setScaleMul:div:)])
                {
                    [delegate setScaleMul:(unsigned)pLine->nParam[0]
                                      div:(unsigned)pLine->nParam[1]];
                }
                break;
            default:
...
- (KmlLine *)InitLcd:(KmlBlock *)pBlock
{
	...
	while (pLine)
	{
        ...
        // v1.68 changes
        case TOK_ZOOM:
            if ([delegate respondsToSelector:@selector(parsedLcdScaleX:scaleY:)])
            {
                unsigned scale = pLine->nParam[0];

                if (scale == 0)
                    scale = 1;

                [delegate parsedLcdScaleX:scale scaleY:scale];
            }
            break;
        // v1.68 changes
        case TOK_ZOOMXY:
            if ([delegate respondsToSelector:@selector(parsedLcdScaleX:scaleY:)])
            {
                unsigned scaleX = pLine->nParam[0];
                unsigned scaleY = pLine->nParam[1];
                ...
                [delegate parsedLcdScaleX:scaleX scaleY:scaleY];
            }
            break;
		case TOK_COLOR:
