...
static KmlTokenC pLexToken[] =
{
    ...
    {TOK_ZOOMXY,     000011, 6,@"Zoomxy"}, // v1.68 changes
    {TOK_LOCALE,     000001, 6,@"Locale"},
    {TOK_IFMEM,      000111, 5,@"IfMem"},
    {TOK_SCALE,      000011, 5,@"Scale"},
    {TOK_ICON,       000002, 4,@"Icon"},
    {TOK_NONE,       000000, 0,@""}, // Must be the last token
};
...
int GetIntegerParam(KmlBlock *pBlock,
                    TokenId eBlock,
                    TokenId eCommand,
                    UINT nParam)
{
    while (pBlock)
    {
        if (pBlock->eType == eBlock)
        {
            ...
            while (pLine)
            {
                if (pLine->eCommand == eCommand)
                {
                    return (int)pLine->nParam[nParam];
                }
...
// Used by calculator gallery
- (CalcRect)LoadKMLBackground:(NSString *)filename
{
    CalcRect background;
    memset(&background, 0, sizeof(background));

    NSString *buf = MapKMLFile(filename);
    if (!buf)
        return background;
    ...
    {
        if (eToken == TOK_BACKGROUND)
        {
            KmlBlock *pBlock =
                [self ParseBlock:(TokenId)eToken];
            ...
            if (pBlock)
            {
                ...
                while (pLine)
                {
                    ...
                    {
                        case TOK_OFFSET:
                            background.origin.x = pLine->nParam[0];
                            background.origin.y = pLine->nParam[1];
                            break;

                        case TOK_SIZE:
                            background.size.width = pLine->nParam[0];
                            background.size.height = pLine->nParam[1];
                            break;
        ...
        /*
         * We don't need any other block. Parse it only so
         * the lexer can advance to the next block.
         */
        if (IsGlobalBlock((TokenId)eToken))
        {
            KmlBlock *pBlock =
                [self ParseBlock:(TokenId)eToken];
        ...
        else if (eToken == TOK_INCLUDE)
        {
            /*
             * For now, let the normal block parser handle
             * includes if they occur before Background.
             */
            NSString *includeFile = nil;

            if ([self Lex:LEX_PARAM] == TOK_STRING)
                includeFile = [NSString stringWithString:szLexString];

            if (includeFile)
            {
                KmlBlock *pBlock =
                    [self IncludeBlocks:includeFile];
...
- (int)integerForBlockId:(TokenId)aBlock
              commandId:(TokenId)aCommand
                atIndex:(unsigned int)aIndex
{
    return GetIntegerParam(block, aBlock, aCommand, aIndex);
}
...
// v1.68 changes
- (void)parsedLcdScaleX:(unsigned)x scaleY:(unsigned)y
{
    lcdScaleX = (x > 0) ? x : 1;
    lcdScaleY = (y > 0) ? y : 1;
}
...
// v1.68 changes
static unsigned ScaleValue(unsigned value, unsigned mul, unsigned div)
{
    if (div == 0)
        return value;

    return (unsigned)(((uint64_t)value * mul) / div);
}

// v1.68 changes
- (void)setScaleMul:(unsigned)mul div:(unsigned)div
{
    scaleMul = (mul > 0) ? mul : 1;
    scaleDiv = (div > 0) ? div : 1;
}
...
- (void)applyScale
{
    if (scaleMul == 0 || scaleDiv == 0)
        return;

    // Scale background position and size.
    background.origin.x =
        ScaleValue(background.origin.x, scaleMul, scaleDiv);
    background.origin.y =
        ScaleValue(background.origin.y, scaleMul, scaleDiv);
    background.size.width =
        ScaleValue(background.size.width, scaleMul, scaleDiv);
    background.size.height =
        ScaleValue(background.size.height, scaleMul, scaleDiv);

    // Scale LCD position.
    lcdOrigin.x =
        ScaleValue(lcdOrigin.x, scaleMul, scaleDiv);
    lcdOrigin.y =
        ScaleValue(lcdOrigin.y, scaleMul, scaleDiv);

    // Scale button coordinates.
    for (unsigned i = 0; i < nButtons; ++i)
    {
        pButton[i].nOx =
            ScaleValue(pButton[i].nOx, scaleMul, scaleDiv);
        pButton[i].nOy =
            ScaleValue(pButton[i].nOy, scaleMul, scaleDiv);
        pButton[i].nDx =
            ScaleValue(pButton[i].nDx, scaleMul, scaleDiv);
        pButton[i].nDy =
            ScaleValue(pButton[i].nDy, scaleMul, scaleDiv);
        pButton[i].nCx =
            ScaleValue(pButton[i].nCx, scaleMul, scaleDiv);
        pButton[i].nCy =
            ScaleValue(pButton[i].nCy, scaleMul, scaleDiv);
    }

    // Scale annunciator coordinates.
    for (unsigned i = 0; i < 6; ++i)
    {
        pAnnunciator[i].nOx =
            ScaleValue(pAnnunciator[i].nOx, scaleMul, scaleDiv);
        pAnnunciator[i].nOy =
            ScaleValue(pAnnunciator[i].nOy, scaleMul, scaleDiv);

        pAnnunciator[i].nDx =
            ScaleValue(pAnnunciator[i].nDx, scaleMul, scaleDiv);
        pAnnunciator[i].nDy =
            ScaleValue(pAnnunciator[i].nDy, scaleMul, scaleDiv);

        pAnnunciator[i].nCx =
            ScaleValue(pAnnunciator[i].nCx, scaleMul, scaleDiv);
        pAnnunciator[i].nCy =
            ScaleValue(pAnnunciator[i].nCy, scaleMul, scaleDiv);
    }
    
    lcdScaleX = lcdScaleX * (CGFloat)scaleMul / (CGFloat)scaleDiv;
    lcdScaleY = lcdScaleY * (CGFloat)scaleMul / (CGFloat)scaleDiv;
}
...
- (void)reloadRom
{
    NSLog(@"KML romPath: %@", romPath);
    NSLog(@"KML patchPath: %@", patchPath);
    NSLog(@"KML current directory: %@",
