...
// Optional Emu48 Mac UI settings appended to the .e48 file.
#define E48_UI_SETTINGS_VERSION 2

static const BYTE e48UISettingsMagic[4] = {
    'U', 'I', '4', '8'
};
...
#define E48_CALC_INFO_VERSION 1

static const BYTE e48CalcInfoMagic[4] = {
    'C', 'I', 'N', 'F'
};
...
BOOL CrcRom(WORD *pwChk)
{
    ...
    dwSize = dwRomSize / sizeof(*pdwData);

    while (dwSize-- > 0)
    {
        DWORD dwData = *pdwData++;

        if ((dwData & 0xF0F0F0F0) != 0)
        {
            return FALSE;
        }
...
{
    self = [super init];
    uiZoomPercent = -1; // autofit
    calculatorInfo = nil; // calc info
...
#define CHECKAREA(s,e) (offsetof(CHIPSET,e)-offsetof(CHIPSET,s)+sizeof(((CHIPSET *)NULL)->e))
    ...
    /*
     * Optional Emu48 Mac UI settings.
     *
     * Older .e48 files end here. If the UI settings block is absent,
     * uiZoomPercent remains 100.
     */
    {
        BYTE magic[sizeof(e48UISettingsMagic)];
        
        if (read(hFile, magic, sizeof(magic)) == sizeof(magic))
        {
            if (memcmp(magic,
                       e48UISettingsMagic,
                       sizeof(e48UISettingsMagic)) == 0)
            {
                E48UISettings settings;
                
                if (read(hFile, &settings, sizeof(settings)) == sizeof(settings) &&
         ...
         *
         * Older .e48 files do not contain this block. If it is absent,
         * calculatorInfo remains nil.
         */
        {
            BYTE magic[sizeof(e48CalcInfoMagic)];

            if (read(hFile, magic, sizeof(magic)) == sizeof(magic))
            {
                if (memcmp(magic,
                           e48CalcInfoMagic,
                           sizeof(e48CalcInfoMagic)) == 0)
                {
                    uint32_t version;
                    uint32_t length;

                    if (read(hFile, &version, sizeof(version)) == sizeof(version) &&
                        read(hFile, &length, sizeof(length)) == sizeof(length) &&
                        version == E48_CALC_INFO_VERSION &&
                        length > 0)
                    {
                        NSMutableData *data =
...
//    NSLog(@"KML PARSE: file=%@ result=%@ error=%@",
//          aFilename,
//          freshKml ? @"SUCCESS" : @"FAILED",
//          (*outError ? [*outError localizedDescription] : @"none"));
...
- (BOOL)saveAs:(NSString *)aStateFile error:(NSError **)outError
{
    ...
    if (Chipset.Port0Size)
        write(hFile, Chipset.Port0, Chipset.Port0Size * 2048);
    
    if (Chipset.Port1Size)
        write(hFile, Chipset.Port1, Chipset.Port1Size * 2048);
    
    if (Chipset.Port2Size)
        write(hFile, Chipset.Port2, Chipset.Port2Size * 2048);
     ...
     * Optional calculator information.
     *
     * Only write this block when calculatorInfo is present.
     */
    if (calculatorInfo)
    {
        NSData *data =
        ...
        uint32_t version = E48_CALC_INFO_VERSION;
        uint32_t length = (uint32_t)[data length];

        write(hFile,
              e48CalcInfoMagic,
              sizeof(e48CalcInfoMagic));
    ...
    // SaveBreakpointList(hCurrentFile);
